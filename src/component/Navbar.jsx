...
import { useNavigate } from "react-router-dom";
import ProfileModal from "./ProfileModal";
import { useCasesData } from "./UseCasesData"; // Your data
...
export default function Navbar({ onMenuClick }) {
  ...
  const handleChange = (e) => {
    ...
    const filtered = useCasesData.filter((useCase) =>
      useCase.title.toLowerCase().includes(value.toLowerCase())
    );
  ...
  // On suggestion click or search button click
  const handleSearch = (title) => {
    const selected = title || searchInput.trim();
    if (!selected) return;

    // Navigate using query param
    navigate(`/use-cases?search=${encodeURIComponent(selected)}`);
